...
同一任务内较高优先级先执行，不同层级的数字不可直接比较。动力学子任务可在积分子步多次执行；IK 只更新缓存目标，轨迹发布器读取缓存，不重复求解。实时 IK 保存末端目标位置和姿态，纯平移严格要求零角速度；关节约束通过整组六维速度统一缩放处理。新实例默认使用 `teleop-balanced-v1` 均衡初态。参考推进只取决于 deadman 与指令是否新鲜，不再被实际跟踪误差限速或暂停；跟踪误差仅作为遥测发布。
