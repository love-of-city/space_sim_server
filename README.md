...
默认新场景使用 `teleop-zero-prepare-v1`：六个旋转关节从 0° 开始并保持；点击
**到达操作姿态**后，先搜索/检查关节路径，再通过现有物理控制器平滑运动。
默认操作角度为 **[-30.2, -67.6, -86.6, 143.2, -85.5, 337.4]°**，可在页面编辑。
实测角度、速度稳定到位后才能遥操作。支持中止、超时和断线保护；重置回零并重新等待按钮。
详见 [流程、限制与验证](docs/ARM_PREPARATION.md)。
