...
from space_arm_platform.scene_targets import (
    ...
    MESH_TARGET_TEMPLATE, SELF_COLLISION_TEMPLATE, TASK_BOX_TEMPLATE, capture_target,
)
...
def test_template_resolves_exact_existing_model_and_persists_collision_identity(tmp_path, template, relative, collision):
    ...
    assert bool(metadata["runtime_warning"]) == (template in (MESH_TARGET_TEMPLATE, SELF_COLLISION_TEMPLATE, TASK_BOX_TEMPLATE))
...
def test_default_changes_collision_only_not_seeded_layout_or_saved_compatibility(tmp_path):
    ...
    assert {t["id"] for t in catalog["templates"]} == {SELF_COLLISION_TEMPLATE, MESH_TARGET_TEMPLATE, GROUND_TARGET_TEMPLATE, LEGACY_TEMPLATE, TASK_BOX_TEMPLATE}
