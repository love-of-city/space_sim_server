...
本轮多目标、异常恢复、三轮网页操作/采集和约 10 分钟连续观察已通过。合并入口为 GitHub PR #4；只有最终提交的 CI 也通过才合并主分支。
...
GitHub 对代码提交 `8c4a7ae4b6f84d3318047a81c136ece5a0d975c3` 的 Basic CI 已重新收集并通过：Python `629 passed, 23 deselected`，前端 `159 passed`，信令 `8 passed`，构建和 Ruff 均通过。23 项为显式排除的专用测试，不是意外跳过；原生动力学与复原已有本机独立进程证据。
...
证据为 `run/arm-acceptance-20260924-round3/result.json` 和同目录三张到位截图；最终 `status=passed`，无清理错误。使用本机 headless Edge，通过部署的 HTTPS 入口执行；每轮均从实际六轴零位启动、检查到位前采集返回 409、到位后用真实键盘事件短暂移动机械臂并确认实测变化。

| 轮次 | 场景 | 中断与恢复 | 到位最大误差（度） | 数据集帧数 |
| --- | --- | --- | --- | --- |
| 1 | `scene-20260924-011752-ed12ec86` | 点击取消、复原、显式重试 | 0.0516 | 180 |
| 2 | `scene-20260924-011935-0e709651` | 真实刷新断线、复原、显式重试 | 0.0448 | 187 |
| 3 | `scene-20260924-012128-ae5a857b` | 正常展开，约 10 分钟连续观察 | 0.0449 | 169 |

- 两次轻量运动中断均约 `0.219 s` 检测到停止；这是包含通信/采样的本次测量，不是所有速度工况的制动时间上限。停止后观察 6 秒没有自动重启，复原后的等待状态也再次观察 6 秒，再显式重试。
- 第三轮连续采样约 `599.984 s`，共 597 次，最大采样间隔约 `1.047 s`；后台与浏览器仿真时钟、视频帧计数持续推进，未触发 5 秒停滞条件。
- 该段浏览器视频帧计数速率平均 `79.68 FPS`、中位数 `79.80 FPS`，区间约 `74.80–85.00 FPS`。配置目标为 90 FPS，但本次实际计数速率约 80，不能据此宣称实际显示恒定 90 FPS，也不能代表其他网络的客户端。
- 三轮遥操作后实测关节最大变化分别约 `0.00435 / 0.00251 / 0.00285 rad`，不是只验证按钮状态。
- 三份数据均为 `complete`；两相机逐帧解码数量分别与该轮 180 / 187 / 169 条 Parquet 状态行一致，帧号和时间戳连续，共 536 条状态、1072 帧 RGB 图像。

数据目录位于 `data/episodes/`：

- `episode-20260924-011913-fc42f32e`
- `episode-20260924-012109-1f350556`
- `episode-20260924-013230-62a9cd09`

这些记录带 `infrastructure`、`acceptance` 标签，结果为 `unknown`，不是成功抓取示范。工具联调前两轮分别暴露异步 UI 状态检查过早、CDP 刷新缺少关闭事件两处验收脚本问题；修正等待条件和断线证据后，最终三轮从头重跑通过。早期失败日志与数据保留，没有删除或伪装为成功。
