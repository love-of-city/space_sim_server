...
def test_new_scenes_default_to_one_millisecond(tmp_path):
    manager = SceneRuntimeManager(None, project_root=tmp_path)
    instance = manager.create_instance(SceneInstanceCreate(seed=17))
    assert instance["runtime"]["dynamics_step_s"] == 0.001


@pytest.mark.parametrize("step", [0, -0.001, 0.002, float("inf"), float("nan")])
