...
def packet(index=1, camera="wrist", session="session-a", all_products=False):
    blob = io.BytesIO()
    Image.fromarray(np.full((32, 32, 3), index * 20, dtype=np.uint8)).save(blob, format="PNG")
    products = {"rgb": blob.getvalue()}
    if all_products:
        products["segmentation"] = blob.getvalue()
        products["depth"] = b"Pf\n32 32\n-1.0\n" + np.ones((32, 32), dtype="<f4").tobytes()
    meta = dict(protocol="bsk-capture/1", camera_id=camera, session_id=session,
                ...
                products=[{"name": name, "file_name": name + (".pfm" if name == "depth" else ".png")}
...
def make_recorder(tmp_path, cameras=("wrist",), products=("rgb",)):
    ...
    meta = recorder.start(EpisodeStart(camera_ids=list(cameras), capture_products=list(products)))
    return recorder, tmp_path / meta["episode_id"]


def test_live_v3_roundtrip_all_modalities_and_two_cameras(tmp_path):
    recorder, root = make_recorder(tmp_path, ("overview", "wrist"), ("rgb", "depth", "segmentation"))
    for index in range(1, 4):
        # Both network arrival orders must work, with real encoded products.
        recorder.record_authoritative_capture(*packet(index, "wrist", all_products=True))
        recorder.record_observation(observation(index), None)
        recorder.record_authoritative_capture(*packet(index, "overview", all_products=True))
    ...
    for index in range(3):
        ...
        np.testing.assert_allclose(row["observation.segmentation.wrist"] * 255, (index + 1) * 20, atol=1e-5)
        assert (root / "lerobot" / row["observation.depth_path.wrist"]).read_bytes().startswith(b"Pf\n")
...
def test_invalid_recording_configuration_rejected():
    ...
    for fields in [{"fps": 24}, {"camera_ids": ["a/b", "a_b"]}, {"capture_products": ["depth"]}]:
