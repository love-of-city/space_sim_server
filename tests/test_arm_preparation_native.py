...
@pytest.mark.parametrize('case,goal_deg', [
    ('default', [0, -67.6, -86.6, 143.2, -85.5, 0]),
    ('positive-base-wrist', [12, -67.6, -86.6, 143.2, -85.5, 18]),
    ('negative-base-wrist', [-12, -67.6, -86.6, 143.2, -85.5, -18]),
    ('deeper-elbow', [0, -70, -90, 145, -85.5, 0]),
    ('wrist-quarter-turn', [0, -67.6, -86.6, 143.2, -85.5, 90]),
], ids=lambda value: value if isinstance(value, str) else None)
def test_zero_start_fixed_sequence_real_joint_controllers(tmp_path, monkeypatch, case, goal_deg):
    ...
    class Client(teleop.SimulationControlClient):
        ...
        def latest_action(self):
            ...
            if len(observations)>6 and not any(o['arm_preparation']['ready'] for o in observations):
                action['arm_preparation']=dict(request_id='native-preparation',joint_position_deg=goal_deg)
            return action,False
        def send_observation(self,message):
            ...
            if len(ready)>=90:raise Finished()
            return True
    ...
    report={'case':case,'goal_deg':goal_deg,'final':observations[-1],'count':len(observations),
        'statuses':sorted({o['arm_preparation']['status'] for o in observations})}
    report_root=Path(os.environ.get('SPACE_SIM_PREPARATION_REPORT_DIR', str(tmp_path)))
    report_root.mkdir(parents=True, exist_ok=True)
    (report_root/f'{case}.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
    ready=[o for o in observations if o['arm_preparation']['ready']]
    assert len(ready)>=90
    assert {'waiting','arming','moving','settling','ready'} <= set(report['statuses'])
    for observation in ready:
        np.testing.assert_allclose(observation['arm_joint_position_rad'],np.deg2rad(goal_deg),atol=np.deg2rad(.8))
        assert np.max(np.abs(observation['arm_joint_velocity_rad_s'])) <= .02
    phase_count=len(instance['arm_preparation_plan']['waypoints_rad'])
    assert ready[0]['arm_preparation']['phase_count'] == phase_count
    assert len(ready[0]['arm_preparation']['phase_times']) == phase_count
    active=[o for o in observations if o['arm_preparation']['status'] in {'arming','moving','settling'}]
    for index in (0,5):
        if goal_deg[index] == 0:
            np.testing.assert_allclose(np.asarray([o['target_arm_joint_position_rad'] for o in active])[:,index],0.,atol=1e-12)
